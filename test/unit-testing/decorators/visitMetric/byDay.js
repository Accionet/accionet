  ...
  it('Check one day delay', (done) => {
    return Place.all().then((places) => {
      ...
      Visits.byDay({
        ...
        Visits.byDay({
          place_id: place.id,
        }, (24 * 60) + bd_offset).then((withOffset) => {
          for (let i = 0; i < noOffset.length; i++) {
            // quantities should not change
            assert.equal(noOffset[i][1], withOffset[i][1]);
            // time difference must be 1 day
            assert.equal((new Date(withOffset[i][0]).getTime() - new Date(noOffset[i][0]).getTime()) / (24 * 60 * 60 * 1000), 1);
          }
          done();
        }).catch((error) => {
          done(error);
